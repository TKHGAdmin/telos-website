...
- **Rate-limit posture is inconsistent**: the public marketing POSTs (`submit-quiz.js`, `submit-email.js`, `submit-chs-application.js`) all rate-limit by IP via `ratelimit:*:{ip}` counters, but the client-auth endpoints (`api/client/login.js`, `api/client/reset-password.js`) do not. Any NEW public POST should be checked against this pattern — the inconsistency itself is a source of bugs (a dev might follow the wrong template).
- **Admin GET endpoints leak sensitive fields**: `clients.js` returns the raw `client:*` record including `passwordHash`/`passwordSalt`. `me.js` correctly strips before responding. When reviewing any new admin GET, check whether it hands the browser fields that only belong server-side. Precedent: `me.js` is the gold standard.
